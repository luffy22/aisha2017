...
class PlgSystemEasyCalcCheckPlus extends JPlugin
{
	...
	private function loadEcc($option, $task, $view, $func, $layout)
	{
		...
		else
		{
			...
			{
				$this->extension_info = array('com_phocaguestbook', '<form.+com_phocaguestbook.*</form>', '<input type=.+>', '<div class="btn-group">\s*<button type="submit".*>', 'submit');
			...
			{
				$this->extension_info = array('com_kunena', '<form[^>]+id="postform".+</form>', '<input type=.+>', '<button[^>]+type="submit"[^>]+>', 'post');
		...
		if($this->load_ecc == true)
		{
			...
			if(($this->params->get('type_calc') OR $this->params->get('recaptcha') OR $this->params->get('question')) AND !empty($match_extension))
			{
				...
				if($this->params->get('type_calc'))
				{
					...
					if($tcalc == 1)
					{
						if($this->params->get('converttostring'))
						{
							if($this->params->get('operand') == 2)
							{
								$add_string .= $this->converttostring($spam_check_1).' + '.$this->converttostring($spam_check_2).' =  ';
							}
							elseif($this->params->get('operand') == 3)
							{
								$add_string .= $this->converttostring($spam_check_1).' + '.$this->converttostring($spam_check_2).' + '.$this->converttostring($spam_check_3).' =  ';
							}
						}
						else
						{
							if($this->params->get('operand') == 2)
							{
								$add_string .= $spam_check_1.' + '.$spam_check_2.' =  ';
							}
							elseif($this->params->get('operand') == 3)
							{
								$add_string .= $spam_check_1.' + '.$spam_check_2.' + '.$spam_check_3.' =  ';
							}
						}
					}
					elseif($tcalc == 2)
				...
				if($this->params->get('recaptcha') AND $this->params->get('recaptcha_publickey') AND $this->params->get('recaptcha_privatekey'))
				{
					...
					$add_string .= recaptcha_get_html($publickey, null, true).'<br />';
				}
