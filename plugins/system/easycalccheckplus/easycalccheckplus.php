...
class PlgSystemEasyCalcCheckPlus extends JPlugin
{
	...
	function __construct(&$subject, $config)
	{
		...
		{
			$this->redirect_url = JFactory::getURI()->toString();
		}
			...
			foreach(explode(',', 'GET,POST,REQUEST') as $name_space)
			{
				...
				if(!empty($name_space))
				{
					foreach($name_space as $k => $v)
					{
						...
						$p_dbprefix = $this->app->getCfg('dbprefix');
		...
		if($this->load_ecc == true)
		{
			...
			if(($this->params->get('type_calc') OR $this->params->get('recaptcha') OR $this->params->get('question')) AND !empty($match_extension))
			{
				...
				if($this->params->get('question') AND $this->params->get('question_q') AND $this->params->get('question_a'))
				{
					...
					$add_string .= '<p>'.$this->params->get('question_q').' <input type="text" name="'.$this->session->get('question', null, 'easycalccheck').'" id="'.$this->session->get('question', null, 'easycalccheck').'" size="'.$size.'" class="inputbox '.$this->random().'" value="" /> *</p>';
				}
			...
			// Set redirect url
			$this->session->set('redirect_url', JFactory::getURI()->toString(), 'easycalccheck');
		}
	}
	...
	private function performChecks()
	{
		...
		if($this->params->get('recaptcha') AND $this->params->get('recaptcha_publickey') AND $this->params->get('recaptcha_privatekey'))
		{
			...
			$resp = recaptcha_check_answer($privatekey, $this->session->get('ip', null, 'easycalccheck'), $request['recaptcha_challenge_field'], $request['recaptcha_response_field']);
	...
	private function loadEcc($option, $task, $view, $func, $layout)
	{
		...
		else
		{
			...
			if($option == 'com_contact')
			{
				...
				$this->extension_info = array('com_contact', '<form[^>]+id="contact-form".+</form>', '<label id="jform_contact.+>', '<button class="btn btn-primary validate" type="submit">', 'contact.submit');
			...
			elseif($option == 'com_users')
			{
				if($layout != 'confirm' AND $layout != 'complete')
				{
					...
					{
						$this->extension_info = array('com_users', '<form[^>]+id="member-registration".+</form>', '<label id="jform.+>', '<button type="submit" class="validate">', 'registration.register');
					}
					elseif($view == 'reset' OR $view == 'remind')
					{
						$this->extension_info = array('com_users', '<form[^>]+id="user-registration".+</form>', '<label id="jform_email-lbl"', '<button type="submit">', 'registration.register');
					}
					elseif($view == 'login' OR $view == '')
					{
						$this->extension_info = array('com_users', '<form[^>]+task=user.login.+</form>', '<label id=".+"', '<button type="submit" class=".*">', 'registration.register');
					}
			...
			{
				$this->extension_info = array('com_easybookreloaded', '<form[^>]+name=("|\')gbookForm("|\').+</form>', '<input type=.+>', '<p id="easysubmit">', 'save', 'gbookForm, gbvote, gbtext');
			...
			{
				$this->extension_info = array('com_phocaguestbook', '<form[^>]+id="pgbSaveForm".+</form>', '<input type=.+>', '<input type="submit" name="save" value=".+" />', 'submit');
			...
			elseif($option == 'com_comprofiler') // Community Builder - tested with version 1.9
			{
				if($task == 'registers')
				{
					$this->extension_info = array('com_comprofiler', '<form[^>]+id="cbcheckedadminForm".+</form>', '<label for=".+>', '<input type="submit" value=".+" class="button" />', 'saveregisters');
				}
				elseif($task == 'lostpassword')
				{
					$this->extension_info = array('com_comprofiler', '<form[^>]+id="adminForm".+</form>', '<label for=".+>', '<input type="submit" class="button" id="cbsendnewuspass" value=".+" />', 'sendNewPass');
				}
			...
			{
				$this->extension_info = array('com_dfcontact', '<form[^>]+id="dfContactForm".+</form>', '<label for="dfContactField.+>', '<input type="submit" value=".+" class="button" />');
			...
			{
				$this->extension_info = array('com_foxcontact', '<form[^>]+id="FoxForm".+</form>', '<input class=.+>', '<input class="foxbutton" type="submit" style=".+" name=".+" value=".+"/>');
			...
			elseif($option == 'com_flexicontact' OR $option == 'com_flexicontactplus') // FlexiContact - tested with version 5.12 / FlexiContact Plus - tested with version 6.07
			{
				...
				$this->extension_info = array($option, '<form[^>]+name="fc.?_form".+</form>', '<input type=.+>', $regex_output, 'send');
			...
			{
				$this->extension_info = array('com_kunena', '<form[^>]+id="postform".+</form>', '<input type=.+>', '<input type="submit" name="ksubmit" class="kbutton".+/>', 'post');
			...
			{
				$this->extension_info = array('com_alfcontact', '<form[^>]+id="contact-form".+</form>', '<label for=".+>', '<button class="button">', 'sendemail');
			...
			{
				$this->extension_info = array('com_aicontactsafe', '<form[^>]+id="adminForm_.+</form>', '<label for=".+>', '<input type="submit" id="aiContactSafeSendButton"', 'display');
			...
			{
				$this->extension_info = array('com_community', '<form[^>]+id="jomsForm".+</form>', '<label id=".+>', '<div[^>]+cwin-wait.*></div>', 'register_save');
			...
			elseif($option == 'com_virtuemart') // Virtuemart - tested with version 2.0.24a
			{
				...
				{
					$this->extension_info = array('com_virtuemart', '<form[^>]+id="askform".+</form>', '<label>', '<input[^>]*type="submit" name="submit_ask"[^>]*/>', 'mailAskquestion');
				...
				{
					$this->extension_info = array('com_virtuemart', '<form[^>]+id="userForm".+</form>', '<label.+>', '<button[^>]*type="submit"[^>]*>', 'savecheckoutuser');
				...
				{
					$this->extension_info = array('com_virtuemart', '<form[^>]+id="userForm".+</form>', '<label.+>', '<button[^>]*type="submit"[^>]*>', 'savecartuser');
				...
				{
					$this->extension_info = array('com_virtuemart', '<form[^>]+name="userForm".+</form>', '<label.+>', '<button[^>]*type="submit"[^>]*>', 'saveUser');
			...
			elseif($option == 'com_iproperty') // IProperty - tested with version 3.3
			{
				$this->extension_info = array('com_iproperty', '<form[^>]+name="sendRequest".+</form>', '<label id=".+>', '<button[^>]*type="submit"[^>]*>', 'property.sendRequest');

				if($this->params->get('iproperty') AND $view == 'property' AND $task == '')
				{
					$this->load_ecc = true;
				}
				elseif($this->params->get('iproperty') AND $view == 'property' AND $task == 'property.sendRequest')
				{
					$this->load_ecc_check = true;
				}
			}
	...
	private function customCall()
	{
		...
		if(preg_match("@(<form[^>]*>)(.*)({easycalccheckplus})(.*</form>)@Us", $body, $matches))
		{
			...
			if(!empty($matches))
			{
				...
				// Array -> (name, form, regex for hidden field, regex for output, task, request exception for encode option);
				$this->load_ecc = true;
				$this->extension_info = array($this->request->get('option', '', 'WORD'), preg_quote($custom_call_form), $custom_call_hidden_regex, '{easycalccheckplus}', $custom_call_form_task);
